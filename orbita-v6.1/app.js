import { ensureSeed, saveAutoBackup, enableWriteThroughAutoBackup } from './db.js?v=6.1';
import { $, $$, toast, initTooltips, animateStatCards, performanceProfile } from './helpers.js?v=6.1';
import * as V from './views.js?v=6.1';
...
function installGlobalErrorHandlers() {
  ...
  window.addEventListener('error', (event) => {
    console.error('Erro global do aplicativo:', event.error || event.message);
  });
}
...
async function startApp() {
  ...
  wireNav();
  wireGlobalActions();
  // v6.1: nenhuma migração automática toca em registros/históricos ao abrir o app.
  // Correções de dados antigos só acontecem por ação explícita do usuário.
...
async function openOperatorPicker() {
  const { Collaborators } = await import('./db.js?v=6.1');
...
export function openModal({ title, subtitle = '', body, actions = [] }) {
  ...
  $$('#modalActions button').forEach((btn, i) => {
    ...
    btn.addEventListener('click', async () => {
      ...
      try {
        await actions[i].onClick();
      } catch (err) {
        console.error('Falha em ação do modal:', err);
        toast(`Não foi possível concluir esta ação${err?.message ? `: ${err.message}` : '.'}`, 'error');
      } finally {
        busy = false;
...
async function render() {
  ...
  try {
    ...
    console.error(`Falha ao renderizar ${currentView}:`, err);
    view.innerHTML = `<div class="empty-state system-error-state"><strong>Não foi possível abrir esta área</strong><span>Os dados permanecem salvos. Tente novamente ou volte à Central Operacional.</span><small>${String(err?.message || 'Erro inesperado').replace(/[&<>"']/g, '')}</small></div>`;
    toast('Ocorreu um erro nesta tela. Seus dados não foram apagados.', 'error');
  }
}

async function updateBadges() {
  const { Deliveries, Cycles } = await import('./db.js?v=6.1');
...
if ('serviceWorker' in navigator) {
  ...
  window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=6.1', { updateViaCache: 'none' }).then((registration) => registration.update()).catch(() => {}));
}
