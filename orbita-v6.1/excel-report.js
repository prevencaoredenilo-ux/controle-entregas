import { Deliveries, Vehicles, Drivers, Collaborators, Neighborhoods, CostCategories, Cycles, OdometerLogs, Costs, DayClosures } from './db.js?v=6.1';
import { STATUS_META } from './helpers.js?v=6.1';
...
function hasOpenRetry(r){ const attempts=r.returnAttempts||[]; const last=attempts.at(-1); return r.status==='na_loja' && !!last?.retryPlanned; }
function isFinal(r){ return r.status==='finalizada'; }
function isRevenue(r){ return !!r && !r.deletedAt; }
function slaBase(r){ return r.type==='agendada'&&r.scheduledAt?r.scheduledAt:r.entryTime; }
function startLate(r){
  const base=slaBase(r); if(!base)return false;
  const active=['na_loja','em_rota','no_cliente'].includes(r.status);
  if(!r.leftStoreAt&&!active)return false;
  const end=r.leftStoreAt?new Date(r.leftStoreAt).getTime():Date.now();
  const begin=new Date(base).getTime();
  return Number.isFinite(end)&&Number.isFinite(begin)&&end-begin>targets().startMinutes*60000;
}
function arrivalLate(r){
  const base=slaBase(r); if(!base)return false;
  const active=['na_loja','em_rota','no_cliente'].includes(r.status);
  const endpoint=r.clientArrivalAt||r.deliveredAt;
  if(!endpoint&&!active)return false;
  const end=endpoint?new Date(endpoint).getTime():Date.now();
  const begin=new Date(base).getTime();
  return Number.isFinite(end)&&Number.isFinite(begin)&&end-begin>targets().arrivalMinutes*60000;
}
...
function currentSituation(r){
  ...
  if(hasOpenRetry(r))return'Na loja · nova tentativa';
  return'Na loja';
}
...
function nextAction(r){
  ...
  if(hasOpenRetry(r)) return r.nextAttemptAt?`Nova tentativa ${dateTimeText(r.nextAttemptAt)}`:'Nova tentativa pendente';
...
function dailyMetrics(dayRows, dayCosts, dayCycles, dayKm){
  ...
  return {
    ...
    rescheduledOpen:dayRows.filter((r)=>r.status==='reentrega'||hasOpenRetry(r)).length,
    ...
    firstAttempt:percent(delivered.filter((r)=>(r.returnAttempts||[]).length===0).length,delivered.length),
...
export async function exportFullExcelReport(env, period){
  ...
  add('OCORRENCIAS',['Motivo ou ocorrência','Registros','Compras afetadas','Devoluções','Reagendamentos','Cancelamentos','Bairro mais frequente','Entregador mais frequente','Veículo mais frequente'],[...occMap.entries()].map(([label,gr])=>[label,gr.length,unique(gr.map((r)=>r.id)).length,gr.filter((r)=>['retorno','reentrega'].includes(r.status)||(r.returnAttempts||[]).length).length,gr.filter((r)=>(r.reschedules||[]).length||r.status==='reentrega'||hasOpenRetry(r)).length,gr.filter((r)=>r.status==='cancelada').length,mode(gr.map((r)=>nName(r.neighborhoodId))),mode(gr.map((r)=>dName(r.driverId))),mode(gr.map((r)=>vName(r.vehicleId)))]));
