import { Deliveries, Vehicles, Drivers, Collaborators, Neighborhoods, CostCategories, ReturnReasons, Cycles, OdometerLogs, Costs, DayClosures, AuditLog, Counters } from './db.js?v=6.1';
import { $, $$, money, dateBR, dateTimeBR, timeBR, escapeHtml, toast, badge, STATUS_META, guardClick, downloadCSV, downloadJSON, wirePhoneMask, animateStatCards, motivationalPhrase, performanceProfile, barChartSVG, lineChartSVG, thermometerHTML } from './helpers.js?v=6.1';
import { getEnv, getOperatorName, getOperatorRole, canPerform, closeModal, openModal, refreshApp } from './app.js?v=6.1';
import { exportFullExcelReport } from './excel-report.js?v=6.1';
...
export async function normalizeReturnQueueStatus() {
  // Compatibilidade apenas: v6.1 não migra status antigos ao abrir o sistema.
  return { changed: 0 };
}
...
function isFeeRecognized(record) {
  // A taxa vira receita no momento em que a compra é registrada.
  // O status da entrega não interfere no reconhecimento da receita;
  // eventual reembolso é descontado separadamente.
  return !!record && !record.deletedAt;
}

function grossFeesOf(rows) {
  return rows.filter(isFeeRecognized).reduce((sum, row) => sum + Number(row.deliveryFee || 0), 0);
}

function refundedFeesOf(rows) {
  return rows.filter((row) => row.refunded === true).reduce((sum, row) => sum + Number(row.deliveryFee || 0), 0);
}

function netFeesOf(rows) {
  return grossFeesOf(rows) - refundedFeesOf(rows);
}
...
function nextDailyPurchaseNumber(deliveries, entryTime, excludeId = null) {
  ...
  const used = new Set(dayRows
    .map((item) => Number(item.purchaseNumber))
    .filter((value) => Number.isInteger(value) && value > 0));

  // A sequência é definida pela quantidade/ordem de registros da DATA DA COMPRA,
  // independentemente de a entrega ser para hoje ou agendada para outro dia.
  // Isso evita que um número antigo incorreto e muito alto (ex.: #27) force a próxima compra para #28.
...
async function repairNormalPurchaseNumbers({ environment = getEnv(), selectedDay = null, reason = 'Correção manual da numeração diária pela ordem de cadastro' } = {}) {
  const allRows = (await Deliveries.all()).filter((r) => r.environment === environment);
  const rows = allRows.filter((r) => !r.deletedAt);
  const datedRows = rows.filter((r) => !selectedDay || localDateKey(r.entryTime) === selectedDay);
  ...
  for (const [day, dayRows] of groups.entries()) {
    ...
    const reserved = new Set([
      ...
      ...allRows
        .filter((row) => row.deletedAt && localDateKey(row.entryTime) === day && Number.isInteger(Number(row.purchaseNumber)) && Number(row.purchaseNumber) > 0)
        .map((row) => Number(row.purchaseNumber)),
    ]);
    ...
    for (const row of dayRows) {
      if (row.purchaseNumberManual === true && Number.isInteger(Number(row.purchaseNumber)) && Number(row.purchaseNumber) > 0) {
        ...
        if (Number(row.arrivalNumber) !== Number(row.purchaseNumber)) {
          await Deliveries.update(row.id, { arrivalNumber: Number(row.purchaseNumber) });
          changed += 1;
          changedDays.add(day);
        }
        continue;
...
export async function normalizeExistingDailyPurchaseNumbers() {
  // Mantido por compatibilidade. v6.1 não altera registros existentes automaticamente.
...
function deliverySla(record, nowMs = Date.now()) {
  ...
  const clientDoneValue = record.deliveredAt || record.clientArrivalAt;
  const arrivalMs = clientDoneValue ? new Date(clientDoneValue).getTime() : null;
  return {
...
export async function renderCentral() {
  ...
  const todayFees = grossFeesOf(todayRows);
  const todayRefunds = refundedFeesOf(todayRows);
  ...
  return `
    ...
    <section class="control-room-grid">
      ...
      <article class="ops-pulse-console" id="opsSlaConsole" role="button" tabindex="0" data-start-late="${startLateToday}" data-arrival-late="${arrivalLateToday}" data-risk="${startRiskRows.length+arrivalRiskRows.length}" data-avg-total="${formatDuration(avgTodayTotal)}" data-avg-route="${formatDuration(avgTodayRoute)}" data-tip="Clique para detalhar os dois tipos de atraso e os riscos do dia.">
        ...
        <div class="pulse-metrics">
          ...
          <div data-tip="Tempo médio entre a saída da loja e a entrega ao cliente."><small>Em rota</small><strong>${formatDuration(avgTodayRoute)}</strong></div>
    ...
    <div class="attention-grid sla-attention-grid">
      ...
      <button data-central-filter="atraso_chegada" class="attention-card ${arrivalLateRows.length ? 'critical' : ''}" data-tip="Entregas que não foram concluídas no cliente em até 3 horas e 30 minutos da entrada."><span>⌂</span><div><small>ATRASO NA ENTREGA</small><strong data-count="${arrivalLateRows.length}">0</strong><em>limite: 3h30</em></div></button>
    ...
    <section class="operation-launcher">
      ...
      <div class="command-dock">
        ...
        <button id="qaArrival" data-tip="Registrar uma única vez a hora em que a entrega foi concluída no cliente."><span class="cmd-ico">📍</span><strong>Entregue ao cliente</strong></button>
...
async function miniList(rows) {
  ...
  const trs = rows.slice(0, 30).map((r) => {
    ...
    <tr data-id="${r.id}" class="row-click" data-status="${r.status}" data-priority="${r.priority}" data-late-start="${sla.startLate}" data-late-arrival="${sla.arrivalLate}" data-risk-sla="${sla.startRisk||sla.arrivalRisk}" data-search="${escapeHtml([r.purchaseNumber, r.coupon, r.pdv, r.doc, r.clientName, r.street].join(' ').toLowerCase())}" data-tip="Compra #${r.purchaseNumber} · ${escapeHtml(r.clientName || 'Cliente sem nome')} · Cupom ${escapeHtml(r.coupon || 'não informado')} · PDV ${escapeHtml(r.pdv || '—')} · DOC ${escapeHtml(r.doc || '—')} · Saída limite ${timeBR(sla.startDeadline)} · Entrega limite ${timeBR(sla.arrivalDeadline)} · Saída real ${timeBR(r.leftStoreAt)} · Entrega ao cliente ${timeBR(r.deliveredAt || r.clientArrivalAt)}">
      ...
      <td>${badge(r.status)}${sla.startLate?'<span class="badge problema sla-mini-badge">Saída atrasada</span>':''}${sla.arrivalLate?'<span class="badge problema sla-mini-badge">Entrega atrasada</span>':''}${sla.startRisk||sla.arrivalRisk?'<span class="badge pendente sla-mini-badge">Perto do limite</span>':''}<br><span class="delivery-times">Limites: saída ${timeBR(sla.startDeadline)} · entrega ${timeBR(sla.arrivalDeadline)}</span></td>
      <td>${r.priority === 'alta' ? '<span class="badge problema">Alta</span>' : '—'}</td>
      <td>${money(r.deliveryFee)}</td>
      <td>${r.status === 'em_rota' ? `<div class="queue-actions"><button class="btn-primary btn-small arrival-row-btn" data-id="${r.id}">Entregue</button><button class="btn-ghost btn-small return-row-btn" data-id="${r.id}">Retornou</button></div>` : r.status === 'no_cliente' ? `<div class="queue-actions"><button class="btn-primary btn-small completion-row-btn" data-id="${r.id}">Confirmar</button><button class="btn-ghost btn-small return-row-btn" data-id="${r.id}">Retornou</button></div>` : '<span style="color:var(--text-muted)">—</span>'}</td>
    </tr>`;
  }).join('');
  ...
  openModal({
    ...
    body: `<form id="recalculateNumberingForm" class="recalculate-numbering-form">
      ...
      <div class="numbering-rule-card">
        <span>↻</span>
        <div><strong>Regra da sequência</strong><p>A numeração segue a ordem em que as compras foram cadastradas para a data escolhida. Entrega normal e agendada usam a MESMA sequência do dia da compra. A data agendada para entrega não muda o número. A hora da compra não interfere e correções manuais não são sobrescritas.</p></div>
      </div>
    </form>`,
    actions: [
...
export function wireCentralEvents() {
  ...
  activateCard('#opsSlaConsole', () => {
    ...
    openCentralInsightDrawer('Tempos e SLA de hoje',`<div class="drawer-kpi-grid"><div><small>Atraso de saída</small><strong>${card.dataset.startLate}</strong><span>limite 2 horas</span></div><div><small>Atraso na entrega</small><strong>${card.dataset.arrivalLate}</strong><span>limite 3h30</span></div><div><small>Perto do limite</small><strong>${card.dataset.risk}</strong><span>aviso 30 min antes</span></div><div><small>Tempo completo médio</small><strong>${card.dataset.avgTotal}</strong><span>entrada até finalização</span></div></div><button class="btn-primary drawer-nav-dashboard">Investigar no Centro de Inteligência</button>`);
...
async function openArrivalPicker() {
  ...
  if (!rows.length) return toast('Não há entrega em rota aguardando conclusão no cliente.', 'error');
  openModal({
    title: 'Registrar entrega ao cliente',
    subtitle: 'Escolha qual entrega foi concluída no cliente.',
...
function operationalTimeFields(record) {
  ...
  return `
    <fieldset class="operational-times-card">
      ...
      <p>A partir da v6.1 existe um único horário no cliente: o momento em que a entrega foi concluída. Alterações manuais ficam registradas na auditoria.</p>
      <div class="field-row">
        <label>Saída da loja<input type="datetime-local" name="leftStoreAt" value="${localDateTimeValue(record.leftStoreAt)}" /></label>
        <label>Entrega ao cliente<input type="datetime-local" name="deliveredAt" value="${localDateTimeValue(record.deliveredAt || record.clientArrivalAt)}" /></label>
      </div>
    </fieldset>`;
}
...
export async function openDeliveryModal(record = null) {
  ...
  const env = getEnv();
  const neighborhoods = (await Neighborhoods.all()).filter((n) => n.active !== false);
  const vehicles = (await Vehicles.all()).filter((v) => v.active !== false);
  const drivers = (await Drivers.all()).filter((d) => d.active !== false);
  const numberingRows = (await Deliveries.all()).filter((item) => item.environment === env);
  const initialPurchaseNumber = record ? Number(record.purchaseNumber) || 1 : nextDailyPurchaseNumber(numberingRows, new Date());

  const body = `
    <form id="deliveryForm">
      <div class="delivery-number-single">
        <label>Nº da entrega ${record ? '*' : ''}
          ${record
            ? `<input type="number" min="1" step="1" name="purchaseNumberManual" required value="${initialPurchaseNumber}" />`
            : `<input id="purchaseNumberPreview" value="${initialPurchaseNumber}" readonly aria-label="Próximo número automático da entrega" />`}
        </label>
        <small>${record
          ? 'Pode corrigir manualmente. O sistema bloqueia duplicidade na mesma data da compra e registra a alteração na auditoria.'
          : 'Número automático da compra. Ele já aparece antes de salvar e segue a sequência da data da compra, inclusive para entregas agendadas.'}</small>
      </div>
      <div class="field-row field-row-date-time">
        ...
        <small class="field-row-help">Esta é a data que define a sequência do Nº da entrega. Se a compra for agendada para amanhã, ela continua usando a numeração de hoje.${record ? ' Ao mudar a data da compra, o sistema valida a sequência desse dia.' : ''}</small>
      </div>
      ...
      <label id="scheduledWrap" class="${record?.type === 'agendada' ? '' : 'hidden'}">Data/hora agendada para entrega *
        <input name="scheduledAt" type="datetime-local" value="${record?.scheduledAt ? localDateTimeValue(record.scheduledAt) : ''}" />
      </label>
      <div class="field-row">
        <label>Veículo
  ...
  if (!record) {
    const updatePurchaseNumberPreview = () => {
      const dateValue = $('input[name="operationDate"]')?.value;
      if (!dateValue) return;
      const [year, month, day] = dateValue.split('-').map(Number);
      const date = new Date(year, (month || 1) - 1, day || 1, 12, 0, 0, 0);
      const preview = $('#purchaseNumberPreview');
      if (preview) preview.value = nextDailyPurchaseNumber(numberingRows, date);
    };
    $('input[name="operationDate"]')?.addEventListener('change', updatePurchaseNumberPreview);
    updatePurchaseNumberPreview();
  }
...
function deliveryStatusActionsHtml(record) {
  ...
  if (record.status === 'em_rota') {
    buttons.push('<button type="button" class="btn-primary btn-small" data-action="chegou_cliente">Entregue ao cliente</button>');
  ...
  if (record.status === 'programada' || (record.type === 'agendada' && record.status === 'na_loja')) {
...
function openClientArrivalFlow(record) {
  openModal({
    ...
    subtitle: `Entrega #${record.purchaseNumber} · informe uma única vez o horário em que a entrega foi concluída.`,
    body: `
      <form id="clientArrivalForm" class="delivery-completion-form">
        <div class="single-client-time-info"><span>✓</span><div><strong>Um único horário no cliente</strong><small>Chegada e finalização não serão mais solicitadas separadamente.</small></div></div>
        <label>Hora da entrega ao cliente *<input type="datetime-local" name="deliveredAt" required value="${localDateTimeValue(record.deliveredAt || record.clientArrivalAt || new Date())}" /></label>
      </form>`,
    actions: [
      { label: 'Cancelar', kind: 'ghost', onClick: closeModal },
      { label: 'Confirmar entrega', kind: 'primary', onClick: async () => {
        const form = $('#clientArrivalForm');
        if (!form.reportValidity()) return;
        const fd = Object.fromEntries(new FormData(form).entries());
        const current = await Deliveries.get(record.id);
        if (!current || !['em_rota','no_cliente'].includes(current.status)) return toast('Esta entrega não está mais aguardando conclusão. Atualize a tela.', 'error');
        const deliveryDate = new Date(fd.deliveredAt);
        if (Number.isNaN(deliveryDate.getTime())) return toast('Informe uma data/hora válida para a entrega.', 'error');
        const deliveredAt = deliveryDate.toISOString();
        if (current.leftStoreAt && new Date(deliveredAt) < new Date(current.leftStoreAt)) return toast('A entrega ao cliente não pode ser anterior à saída da loja.', 'error');
        await Deliveries.changeStatus(record.id, 'finalizada', {
          clientArrivalAt: deliveredAt,
          deliveredAt,
          arrivalRegisteredBy: current.arrivalRegisteredBy || getOperatorName(),
          completionRegisteredBy: getOperatorName(),
          completionUpdatedAt: new Date().toISOString(),
          note: `Entrega concluída no cliente às ${timeBR(deliveredAt)}.`,
        });
        toast(`Entrega #${current.purchaseNumber} finalizada às ${timeBR(deliveredAt)}.`, 'success');
        closeModal();
        refreshApp();
      }},
    ],
...
function openDeliveryCompletionFlow(record, { cycle = null, editing = false } = {}) {
  const currentValue = record.deliveredAt || record.clientArrivalAt || new Date();
  openModal({
    title: editing ? 'Editar horário da entrega' : 'Concluir entrega',
    subtitle: editing
      ? `Entrega #${record.purchaseNumber} · corrija o horário único registrado no cliente.`
      : `Entrega #${record.purchaseNumber} · informe uma única vez o horário em que a entrega foi concluída.`,
    body: `
      <form id="deliveryCompletionForm" class="delivery-completion-form">
        <div class="single-client-time-info"><span>✓</span><div><strong>Um único horário no cliente</strong><small>Este horário representa a entrega concluída ao cliente. Não é necessário preencher chegada e finalização separadamente.</small></div></div>
        <label>Hora da entrega ao cliente *
          <input type="datetime-local" name="deliveredAt" required value="${localDateTimeValue(currentValue)}" />
        </label>
    ...
    actions: [
      ...
      { label: editing ? 'Salvar horário' : 'Confirmar entrega', kind: 'primary', onClick: async () => {
        ...
        const fd = Object.fromEntries(new FormData(form).entries());
        const current = await Deliveries.get(record.id);
        if (!current || (!editing && !['em_rota','no_cliente'].includes(current.status)) || (editing && current.status !== 'finalizada')) return toast('O status desta entrega mudou. Atualize a tela antes de concluir.', 'error');
        const deliveryDate = new Date(fd.deliveredAt);
        if (Number.isNaN(deliveryDate.getTime())) return toast('Informe um horário válido para concluir a entrega.', 'error');
        const deliveredAt = deliveryDate.toISOString();
        if (current.leftStoreAt && new Date(deliveredAt) < new Date(current.leftStoreAt)) return toast('A entrega ao cliente não pode ser anterior à saída da loja.', 'error');
        await Deliveries.changeStatus(record.id, 'finalizada', {
          clientArrivalAt: deliveredAt,
          ...
          completionRegisteredBy: getOperatorName(),
          completionUpdatedAt: new Date().toISOString(),
          note: fd.completionNote?.trim() || (editing ? 'Horário da entrega ao cliente corrigido.' : `Entrega concluída no cliente às ${timeBR(deliveredAt)}.`),
        });
...
async function saveDeliveryForm(record) {
  ...
  if (fd.type === 'agendada' && Number.isNaN(new Date(fd.scheduledAt).getTime())) return toast('Informe uma data/hora válida para o agendamento.', 'error');
  ...
  if (record) {
    ...
    payload.clientArrivalAt = payload.deliveredAt || (record.status === 'no_cliente' ? record.clientArrivalAt || null : null);
    if (['em_rota','no_cliente','finalizada'].includes(record.status) && !payload.leftStoreAt) return toast('Esta entrega já saiu da loja e precisa manter a hora de saída.', 'error');
    if (record.status === 'finalizada' && !payload.deliveredAt) return toast('Uma entrega finalizada precisa manter a hora da entrega ao cliente.', 'error');
    if (payload.leftStoreAt && new Date(payload.leftStoreAt) < new Date(payload.entryTime)) return toast('A saída da loja não pode ser anterior à entrada da compra.', 'error');
    ...
    if (payload.deliveredAt && payload.leftStoreAt && new Date(payload.deliveredAt) < new Date(payload.leftStoreAt)) return toast('A entrega ao cliente não pode ser anterior à saída da loja.', 'error');
  }
  ...
  try {
    if (record) {
      ...
      const existingDeliveries = (await Deliveries.all()).filter((item) => item.environment === env);
      ...
      if (manualNumberChanged) {
        const duplicate = existingDeliveries.find((item) => {
          ...
          return localDateKey(item.entryTime) === nextOperationDay;
        });
        if (duplicate) {
          return toast(`Já existe a entrega #${requestedPurchaseNumber} na data da compra ${dateBR(nextOperationDay)}. Escolha outro número.`, 'error');
        }
      ...
      let targetStatus = payload.deliveredAt ? 'finalizada' : (payload.clientArrivalAt && record.status === 'em_rota' ? 'no_cliente' : null);
      if (!targetStatus && ['na_loja','programada'].includes(record.status)) {
        targetStatus = fd.type === 'agendada' ? 'programada' : 'na_loja';
      }
      if (targetStatus && targetStatus !== record.status) await Deliveries.changeStatus(record.id, targetStatus, { note: 'Status ajustado conforme os dados informados na edição.' });
...
async function finishRetirada(record, refunded) {
  ...
  await Deliveries.changeStatus(record.id, 'retirada_loja', {
    pickedUpAtStore: true,
    pickedUpAt: now,
    refunded,
    refundedAt: refunded ? now : null,
    refundedBy: refunded ? getOperatorName() : null,
    note: refunded ? 'Retirada com reembolso' : 'Retirada sem reembolso',
  });
...
async function openReturnResolutionFlow(record, { cycle = null, onBack = null, continueClose = false } = {}) {
  const reasons = (await ReturnReasons.all()).filter((r) => r.active !== false);
  openModal({
    title: 'Registrar retorno à loja',
    ...
    actions: [
      ...
      { label: 'Confirmar retorno', kind: 'primary', onClick: async () => {
        ...
        if (!form.reportValidity()) return;
        const fd = Object.fromEntries(new FormData(form).entries());
        const currentRecord = await Deliveries.get(record.id);
        if (!currentRecord || !['em_rota','no_cliente','finalizada'].includes(currentRecord.status)) return toast('O status desta entrega mudou. Atualize a tela antes de registrar o retorno.', 'error');
        ...
        const returnedDate = new Date(fd.returnedAt);
        if (Number.isNaN(returnedDate.getTime())) return toast('Informe uma data/hora válida para o retorno.', 'error');
        const returnedAt = returnedDate.toISOString();
        ...
        const latestBeforeReturn = record.deliveredAt || record.clientArrivalAt || record.leftStoreAt;
        if (latestBeforeReturn && new Date(returnedAt) < new Date(latestBeforeReturn)) return toast('O retorno não pode ser anterior ao último horário registrado desta entrega.', 'error');
        const retryPlanned = fd.retryPlanned === 'sim';
        if (retryPlanned && !fd.retryAt) return toast('Informe quando será a nova tentativa.', 'error');
        const retryDate = retryPlanned ? new Date(fd.retryAt) : null;
        if (retryDate && Number.isNaN(retryDate.getTime())) return toast('Informe uma data/hora válida para a nova tentativa.', 'error');
        const retryAt = retryDate ? retryDate.toISOString() : null;
  ...
  openModal({
    ...
    actions: [
      ...
      { label: 'Confirmar', kind: 'primary', onClick: async () => {
        ...
        if (!form.reportValidity()) return;
        const fd = Object.fromEntries(new FormData(form).entries());
        const nextDate = new Date(fd.newAt);
        if (Number.isNaN(nextDate.getTime())) return toast('Informe uma data/hora válida para o reagendamento.', 'error');
        await Deliveries.reschedule(record.id, nextDate.toISOString(), fd.reason?.trim() || '');
...
export async function openStartCycleModal() {
  ...
  const vehicles = (await Vehicles.all()).filter((v) => v.active !== false);
  const drivers = (await Drivers.all()).filter((d) => d.active !== false);
  if (!vehicles.length) return toast('Cadastre e ative ao menos um veículo antes de iniciar um ciclo.', 'error');
  if (!drivers.length) return toast('Cadastre e ative ao menos um entregador antes de iniciar um ciclo.', 'error');
  ...
  const body = `
    <form id="startCycleForm">
      ...
      <section class="cycle-selection-panel">
        <div class="cycle-selection-head">
          <div><small>ENTREGAS DO CICLO</small><strong>Selecione e organize a próxima saída</strong></div>
          <span>${candidates.length} disponível(is)</span>
        </div>
        <p class="cycle-scheduled-help">Compras agendadas usam o mesmo número da data da compra e aparecem com a etiqueta AGENDADA. Elas começam desmarcadas para evitar saída antecipada.</p>
        <div id="cycleItemsList" class="cycle-items-list">
          ${candidates.map((d, i) => {
            const scheduled = d.status === 'programada';
            const scheduledLabel = scheduled ? `Para ${dateTimeBR(d.scheduledAt || d.entryTime)}` : 'Disponível agora';
            ...
            <div class="cycle-item ${scheduled ? 'cycle-item-scheduled' : ''}" data-id="${d.id}">
              <label class="cycle-item-check" title="Selecionar entrega">
                <input type="checkbox" ${scheduled ? '' : 'checked'} />
                <span></span>
              </label>
              <div class="cycle-item-copy">
                <div class="cycle-item-title"><strong>#${d.purchaseNumber}</strong><b>${escapeHtml(d.clientName || d.street || 'Sem nome')}</b>${scheduled ? '<span class="cycle-scheduled-badge">AGENDADA</span>' : ''}${d.priority === 'alta' ? '<span class="cycle-priority-badge">ALTA</span>' : ''}</div>
                <div class="cycle-item-meta"><span>📍 ${escapeHtml(neighborhoodName)}</span><span>🕒 ${escapeHtml(scheduledLabel)}</span></div>
              </div>
  ...
  openModal({
    ...
    actions: [
      ...
      { label: 'Iniciar ciclo', kind: 'primary', onClick: async () => {
        ...
        const currentOpenCycles = (await Cycles.all()).filter((c) => c.environment === env && c.status === 'aberto' && !c.deletedAt);
        if (currentOpenCycles.some((c) => c.vehicleId === fd.vehicleId)) return toast('Este veículo acabou de entrar em outro ciclo. Atualize a tela e tente novamente.', 'error');
        if (currentOpenCycles.some((c) => c.driverId === fd.driverId)) return toast('Este entregador acabou de entrar em outro ciclo. Atualize a tela e tente novamente.', 'error');
        const latestSelected = await Promise.all(checked.map((id) => Deliveries.get(id)));
        const unavailable = latestSelected.find((d) => !d || !['na_loja','programada','retorno','reentrega'].includes(d.status) || (d.cycleId && currentOpenCycles.some((c) => c.id === d.cycleId)));
        if (unavailable) return toast('Uma das entregas selecionadas não está mais disponível para este ciclo. Atualize a tela.', 'error');
        const odometerLog = currentKmLogs[0];
        const startDate = new Date(fd.startedAt);
        if (Number.isNaN(startDate.getTime())) return toast('Informe uma data/hora válida para o início do ciclo.', 'error');
        const startedAt = startDate.toISOString();
        const latestEntry = Math.max(...latestSelected.map((d) => new Date(d?.entryTime || 0).getTime()));
...
function isCycleDeliveryResolved(delivery, cycle) {
  ...
  if (delivery.status === 'finalizada') return !!delivery.deliveredAt;
...
function showPendingOne(cycle, delivery, remainingCount) {
  openModal({
    ...
    body: `<div class="cycle-pending-alert"><span>!</span><div><strong>Esta entrega está sem o horário de entrega ao cliente</strong><p>O ciclo permanece bloqueado até você informar o que aconteceu.</p></div></div><div class="cycle-return-question"><span>↩</span><div><strong>Esta entrega voltou? SIM ou NÃO?</strong><small>SIM registra o retorno à loja. NÃO exige somente o horário único da entrega ao cliente. Uma pendência por vez.</small></div></div>`,
    actions: [
...
async function openCycleEndTimeModal(cycle) {
  ...
  const deliveredCycleItems = items.filter((item) => {
    ...
    return !returnedInCycle && item?.status === 'finalizada' && item.deliveredAt;
  });
  ...
  openModal({
    ...
    actions: [
      ...
      { label: 'Confirmar fim do ciclo', kind: 'primary', onClick: async () => {
        ...
        const closeDate = new Date(fd.closedAt);
        if (Number.isNaN(closeDate.getTime())) return toast('Informe uma data/hora válida para o fim do ciclo.', 'error');
        ...
        const currentCycle = await Cycles.get(cycle.id);
        if (!currentCycle || currentCycle.status !== 'aberto') return toast('Este ciclo já foi finalizado ou não está mais disponível.', 'error');
...
async function openKmStartModal() {
  ...
  const vehicles = (await Vehicles.all()).filter((v) => v.active !== false);
  if (!vehicles.length) return toast('Cadastre e ative um veículo antes de registrar quilometragem.', 'error');
  const todayKey = localDateKey();
  ...
  openModal({
    ...
    actions: [
      ...
      { label: 'Salvar correção', kind: 'primary', onClick: async () => {
        ...
        if (oldEnd != null && kmEnd == null) {
          const activeClosure = (await DayClosures.all()).find((c) => c.environment === getEnv() && c.date === log.shiftDate && !c.superseded);
          if (activeClosure) {
            await DayClosures.update(activeClosure.id, {
              superseded: true,
              reopenedAt: now,
              reopenedBy: getOperatorName(),
              reopenReason: `KM histórico reaberto: ${reason}`,
            });
          }
        }
...
export async function renderCosts() {
  ...
  return `
    <div class="stat-row">
      <div class="stat-card"><small>Taxas registradas</small><strong>${money(financial.fees)}</strong></div>
...
async function computeFinancialSummary(env) {
  ...
  const finalized = deliveries.filter((d) => d.status === 'finalizada' || d.status === 'retirada_loja');
  const fees = grossFeesOf(deliveries);
  const refunds = refundedFeesOf(deliveries);
  const costsTotal = (await Costs.all()).filter((c) => c.environment === env && !c.deletedAt).reduce((s, c) => s + Number(c.amount || 0), 0);
  const kmTotal = (await OdometerLogs.all()).filter((l) => l.environment === env && l.kmStart != null && l.kmEnd != null && Number(l.kmEnd) >= Number(l.kmStart)).reduce((s, l) => s + (Number(l.kmEnd) - Number(l.kmStart)), 0);
...
async function openCostModal() {
  ...
  const categories = (await CostCategories.all()).filter((c) => c.active !== false);
  const vehicles = await Vehicles.all();
  const drivers = await Drivers.all();
  openModal({
    ...
    body: `
      <form id="costForm">
        <label>Data *<input type="date" name="date" required value="${localDateKey()}" /></label>
    ...
    actions: [
      ...
      { label: 'Salvar', kind: 'primary', onClick: async () => {
        ...
        if (!Number.isFinite(amount) || amount <= 0) return toast('Informe um valor de custo maior que zero.', 'error');
        await Costs.add({ environment: getEnv(), date: fd.date, categoryId: fd.categoryId, vehicleId: fd.vehicleId || null, driverId: fd.driverId || null, amount, note: fd.note?.trim() || '', deletedAt: null });
...
function byWeekdayCountOccurrences(rows, weekday) {
  ...
  rows.forEach((r) => {
    ...
    if (d.getDay() === weekday) {
      total++;
      datesSeen.add(dashboardDayKey(d));
    }
  });
...
export async function renderDashboard() {
  ...
  const grossFees = grossFeesOf(rows);
  const refunds = refundedFeesOf(rows);
  ...
  const arrivedClient = rows.filter((r) => r.deliveredAt || r.clientArrivalAt).length;
  ...
  const firstAttempt = finalized.filter((r) => (r.returnAttempts || []).length === 0 && !(r.statusHistory || []).some((h) => ['retorno','reentrega'].includes(h.to))).length;
  ...
  const arrivalEvaluated = dashboardSla.filter(({record,sla})=>record.deliveredAt||record.clientArrivalAt||sla.arrivalLate);
  ...
  const arrivalOnTimeRate = arrivalEvaluated.length ? Math.round(arrivalEvaluated.filter(({record,sla})=>(record.deliveredAt||record.clientArrivalAt)&&!sla.arrivalLate).length/arrivalEvaluated.length*100) : 0;
  ...
  const previousGross = grossFeesOf(previousRows);
  const previousRefunds = refundedFeesOf(previousRows);
  ...
  const missingArrivalTime = finalized.filter((r) => !r.deliveredAt && !r.clientArrivalAt).length;
  ...
  return `
    ...
    <section class="intel-section" data-dashboard-group="geral">
      ...
      <div class="intel-metric-grid">
        ...
        ${metric('Atraso na entrega', arrivalLateDashboard.length, 'Não chegaram ao cliente em até 3 horas e 30 minutos da entrada.', arrivalLateDashboard.length?'bad':'good')}
    ...
    <section class="intel-section" data-dashboard-group="tempos">
      ...
      <article class="chart-card full-chart"><div class="chart-title"><div><span>CUMPRIMENTO DE SLA</span><strong>No prazo versus atrasadas</strong></div><small>entregas</small></div>${barChartSVG({labels:['Saída no prazo','Saída atrasada','Chegada no prazo','Entrega atrasada'],values:[Math.max(0,startEvaluated.length-startLateDashboard.length),startLateDashboard.length,Math.max(0,arrivalEvaluated.length-arrivalLateDashboard.length),arrivalLateDashboard.length],color:'var(--status-pendente)'})}</article>
    </section>
    ...
    <section class="intel-section" data-dashboard-group="financeiro">
      ...
      <div class="intel-metric-grid">
        ${metric('Taxas registradas', money(grossFees), 'Soma das taxas no momento em que cada compra é registrada, mesmo antes da finalização.')}
    ...
    <section class="intel-section" data-dashboard-group="qualidade">
      ...
      <div class="intel-metric-grid">
        ...
        ${metric('Sem horário no cliente', missingArrivalTime, 'Finalizadas sem o horário único da entrega ao cliente.', missingArrivalTime?'bad':'good')}
...
async function buildExport(kind, env, period = null) {
  ...
  if (kind === 'deliveries') {
    ...
    const header = ['Nº entrega','Data entrada','PDV','DOC','Cupom','Cliente','Telefone','Endereço','Nº','Complemento','Referência','Bairro','Taxa','Tamanho','Viagens','Prioridade','Tipo','Agendado para','Status','Saída da loja','Limite da saída','SLA da saída','Entrega ao cliente','Limite da entrega','SLA da entrega','Retornou à loja em','Motivo do retorno','Situação da mercadoria','Itens/volumes retornados','Nova tentativa','Próxima tentativa','Quantidade de retornos','Veículo','Entregador','Reembolsado','Observações'];
    const lines = rows.map((r) => {const sla=deliverySla(r);return [r.purchaseNumber, dateTimeBR(r.entryTime), r.pdv, r.doc, r.coupon, r.clientName, r.phone, r.street, r.houseNumber, r.complement, r.reference, nName(r.neighborhoodId), r.deliveryFee, r.size, r.tripCount, r.priority, r.type, r.scheduledAt ? dateTimeBR(r.scheduledAt) : '', STATUS_META[r.status]?.label, r.leftStoreAt ? dateTimeBR(r.leftStoreAt) : '', dateTimeBR(sla.startDeadline),sla.startLate?'Atrasada':'No prazo',r.deliveredAt ? dateTimeBR(r.deliveredAt) : (r.clientArrivalAt ? dateTimeBR(r.clientArrivalAt) : ''),dateTimeBR(sla.arrivalDeadline),sla.arrivalLate?'Atrasada':'No prazo',r.returnedAt ? dateTimeBR(r.returnedAt) : '',r.returnReasonLabel || '',r.merchandiseSituation || '',r.returnedItems || '',r.retryPlanned ? 'Sim' : r.returnedAt ? 'Não' : '',r.nextAttemptAt ? dateTimeBR(r.nextAttemptAt) : '',(r.returnAttempts || []).length, vName(r.vehicleId), dName(r.driverId), r.refunded ? 'Sim' : 'Não', r.notes];});
  ...
  if (kind === 'audit') {
    const rows = (await AuditLog.all()).filter((e) => (!e.environment || e.environment === env) && inReportPeriod(e.at, period));
    const header = ['Quando', 'Operador', 'Perfil', 'Ambiente', 'Entidade', 'Ação', 'ID do registro'];
    const lines = rows.map((e) => [dateTimeBR(e.at), e.operator || 'Não identificado', e.operatorRole || '', e.environment || 'Legado', e.entityTable, e.action, e.entityId]);
  ...
  if (kind === 'resumo') {
    ...
    const financial = {
      fees: grossFeesOf(rows),
      refunds: refundedFeesOf(rows),
    ...
    const lines = [
      ...
      ['Taxas registradas', financial.fees],
...
export function wireReportsEvents() {
  ...
  $$('.export-btn').forEach((btn) => btn.addEventListener('click', async () => {
    ...
    downloadCSV(`orbita-${name}-${env}-${localDateKey()}.csv`, [header, ...lines]);
  ...
  $('#printReportBtn')?.addEventListener('click', async () => {
    ...
    area.innerHTML = `
      ...
      <h3>Financeiro</h3>
      <table border="1" cellpadding="6" style="border-collapse:collapse;width:100%;margin-bottom:16px">
        <tr><td>Taxas registradas</td><td>${money(financial.fees)}</td></tr>
      ...
      <table border="1" cellpadding="6" style="border-collapse:collapse;width:100%">
        <tr><th>Compra</th><th>Cliente</th><th>Saída</th><th>Entrega ao cliente</th></tr>
        ${finalized.length ? finalized.map((r) => `<tr><td>#${r.purchaseNumber}</td><td>${escapeHtml(r.clientName || r.street || '—')}</td><td>${timeBR(r.leftStoreAt)}</td><td>${timeBR(r.deliveredAt || r.clientArrivalAt)}</td></tr>`).join('') : '<tr><td colspan="4">Nenhuma entrega finalizada no período</td></tr>'}
...
export async function renderAudit() {
  const env = getEnv();
  const log = (await AuditLog.all()).filter((e) => !e.environment || e.environment === env);
  if (!log.length) return `<div class="empty-state"><strong>Sem eventos ainda</strong></div>`;
  const rows = log.slice(0, 200).map((e) => `
    <tr><td>${dateTimeBR(e.at)}</td><td>${escapeHtml(e.operator || 'Não identificado')}<br><small class="delivery-times">${escapeHtml(e.operatorRole || '')}</small></td><td>${escapeHtml(e.environment || 'Legado')}</td><td>${escapeHtml(e.entityTable)}</td><td>${escapeHtml(e.action)}</td><td style="font-family:monospace;font-size:11px">${escapeHtml(e.entityId).slice(0,14)}</td></tr>
  `).join('');
  return `<div class="table-wrap"><table><thead><tr><th>Quando</th><th>Operador</th><th>Ambiente</th><th>Entidade</th><th>Ação</th><th>ID</th></tr></thead><tbody>${rows}</tbody></table></div>`;
}
...
export function wireTrashEvents() {
  $$('.restore-btn').forEach((btn) => btn.addEventListener('click', async (e) => {
    const id = e.target.closest('tr').dataset.id;
    const record = await Deliveries.get(id);
    if (!record) return toast('Entrega não encontrada na lixeira.', 'error');
    const active = await Deliveries.active(record.environment);
    const day = localDateKey(record.entryTime);
    const conflict = active.find((item) => item.id !== record.id && Number(item.purchaseNumber) === Number(record.purchaseNumber) && localDateKey(item.entryTime) === day);
    if (conflict) {
      const next = nextDailyPurchaseNumber(active, record.entryTime);
      if (!confirm(`O número #${record.purchaseNumber} já está em uso. Restaurar esta entrega como #${next}?`)) return;
      await Deliveries.update(record.id, {
        purchaseNumber: next,
        purchaseNumberManual: false,
        numberingCorrections: [...(record.numberingCorrections || []), {
          previous: record.purchaseNumber, current: next, day, by: getOperatorName(), at: new Date().toISOString(),
          reason: 'Ajuste necessário para restaurar entrega da lixeira sem duplicidade', mode: 'restore',
        }],
      });
    }
  ...
  openModal({
    ...
    actions: [
      ...
      { label: 'Salvar', kind: 'primary', onClick: async () => {
        ...
        if (!form.reportValidity()) return;
        const fd = Object.fromEntries(new FormData(form).entries());
        const normalizedValue = String(fd[field] || '').trim();
        if (!normalizedValue) return toast('Informe um nome válido.', 'error');
        const duplicate = (await store.all()).find((item) => item.id !== record?.id && String(item[field] || '').trim().toLowerCase() === normalizedValue.toLowerCase());
        if (duplicate) return toast('Já existe um cadastro com esse nome.', 'error');
        const payload = { [field]: normalizedValue, active: record?.active ?? true };
  ...
  openModal({
    ...
    actions: [
      ...
      { label: 'Salvar', kind: 'primary', onClick: async () => {
        ...
        const duplicate = (await Collaborators.all()).find((item) => item.id !== record?.id && String(item.name || '').trim().toLowerCase() === name.toLowerCase());
        if (duplicate) return toast('Já existe um colaborador com esse nome.', 'error');
        const payload = { name, role: fd.role, active: record?.active ?? true };
  ...
  openModal({
    ...
    actions: [
      ...
      { label: 'Salvar', kind: 'primary', onClick: async () => {
        ...
        const plate = String(fd.plate || '').trim().toUpperCase();
        const existingVehicles = await Vehicles.all();
        const duplicateLabel = existingVehicles.find((item) => item.id !== record?.id && String(item.label || '').trim().toLowerCase() === label.toLowerCase());
        if (duplicateLabel) return toast('Já existe um veículo com esse apelido.', 'error');
        if (plate) {
          const duplicatePlate = existingVehicles.find((item) => item.id !== record?.id && String(item.plate || '').trim().toUpperCase() === plate);
          if (duplicatePlate) return toast('Já existe um veículo com essa placa.', 'error');
        }
        const payload = {
          label, brand: fd.brand?.trim() || '', model: fd.model?.trim() || '',
          plate, year: fd.year || '', type: fd.type, capacity: fd.capacity ? Number(fd.capacity) : null,
...
export async function renderSettings() {
  ...
  const { listAutoBackups } = await import('./db.js?v=6.1');
  ...
  const visibleAutoBackups = autoBackups.slice(0, 100);
  const autoList = visibleAutoBackups.length
    ? visibleAutoBackups.map((b) => `
  ...
  return `
    <div class="stat-card" style="margin-bottom:16px">
      ...
      <p style="font-size:12.5px;color:var(--text-muted);margin:8px 0">O sistema cria um backup sempre que uma informação é adicionada, editada ou excluída e também faz um snapshot de segurança a cada 1 minuto. A v6.1 não apaga backups anteriores automaticamente.</p>
      ${autoBackups.length > visibleAutoBackups.length ? `<p class="backup-preserve-note">Mostrando os ${visibleAutoBackups.length} backups mais recentes de ${autoBackups.length} preservados neste aparelho.</p>` : ''}
      ${autoList}
    </div>
    <div class="stat-card" style="margin-bottom:16px">
...
export function wireSettingsEvents() {
  $$('.auto-restore-btn').forEach((btn) => btn.addEventListener('click', async () => {
    if (!confirm('Restaurar esse backup automático vai substituir os dados atuais. Um backup de segurança será criado antes. Continuar?')) return;
    try {
      const { restoreAutoBackup } = await import('./db.js?v=6.1');
      await restoreAutoBackup(btn.dataset.id);
      toast('Backup automático restaurado. O histórico de backups foi preservado.', 'success');
      refreshApp();
    } catch (err) {
      console.error(err);
      toast(`Não foi possível restaurar este backup${err?.message ? `: ${err.message}` : '.'}`, 'error');
    }
  }));
  $('#settingsBackupBtn')?.addEventListener('click', async () => {
    const data = await (await import('./db.js?v=6.1')).exportAll();
    downloadJSON(`orbita-backup-completo-${localDateKey()}.json`, data);
  ...
  $('#settingsRestoreInput')?.addEventListener('change', async (e) => {
    ...
    const currentBackup = await (await import('./db.js?v=6.1')).exportAll();
    downloadJSON(`orbita-backup-seguranca-antes-restauracao-${Date.now()}.json`, currentBackup);
    try {
      const text = await file.text();
      const data = JSON.parse(text);
      await (await import('./db.js?v=6.1')).importAll(data);
      toast('Backup restaurado.', 'success');
      refreshApp();
    } catch (err) { console.error(err); toast(`Arquivo de backup inválido${err?.message ? `: ${err.message}` : '.'}`, 'error'); }
    e.target.value = '';
  });
