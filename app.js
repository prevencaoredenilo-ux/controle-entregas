import { ensureSeed, saveAutoBackup, enableWriteThroughAutoBackup } from './db.js?v=4.1';
import { $, $$, toast, initTooltips, animateStatCards, performanceProfile } from './helpers.js?v=4.1';
import * as V from './views.js?v=4.1';
...
async function openOperatorPicker() {
  const { Collaborators } = await import('./db.js?v=4.1');
...
async function updateBadges() {
  const { Deliveries, Cycles } = await import('./db.js?v=4.1');
...
if ('serviceWorker' in navigator) {
  ...
  window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=4.1', { updateViaCache: 'none' }).then((registration) => registration.update()).catch(() => {}));
}
