(() => {
  ...
  function migrateState(data) {
    ...
    for (const key of ['deliveries','cycles','odometerLogs','costs','dayClosures']) {
      merged[key].forEach(item => {
        ...
        if (key === 'deliveries') {
          for (const field of ['docNo','cashierNo','customerName','customerPhone','scheduledTime','scheduleNotes','returnReasonId','returnReasonText']) {
    ...
    merged.neighborhoods.forEach(item => { item.name = uppercaseName(item.name); });
    ...
    return `<div class="operation-card-grid v11-operation-cards">${sorted.map(d => {
      ...
      return `<article class="delivery-action-card clear-card v11-delivery-card ${deliveryStatusClass(d.status)} ${liveDelayed && !d.departureTime ? 'late':''}">
        <div class="v11-delivery-head">
          ...
          <div class="v11-delivery-head-status">${statusBadge(d.status)}${liveDelayed && !d.departureTime?'<span class="badge red">Saída fora</span>':''}${liveCompletionDelayed && !d.finalizationTime?'<span class="badge red">Entrega fora</span>':''}</div>
        </div>
  ...
  function cycleTable(list) {
    ...
    return `<div class="table-wrap"><table><thead><tr><th>Data</th><th>Ciclo</th><th>Veículo</th><th>Entregador</th><th>Saída</th><th>Retorno</th><th>Levadas</th><th>Entregues</th><th>Voltaram</th><th>Média KM/ciclo do dia</th><th>Tempo</th><th>Ações</th></tr></thead><tbody>${list.map(c => { const x=cycleCalc(c); return `<tr>
      <td>${dateBR(c.date)}</td><td><div class="cell-title">${esc(c.code)}</div><div class="cell-sub">${c.autoGenerated?'<span class="badge blue">Automático</span> ':''}${c.returnTime?'Fechado':'Em rota'}</div></td><td>${esc(vehicle(c.vehicleId)?.name || '—')}</td><td>${esc(employee(c.driverId)?.name || '—')}</td><td>${c.departureTime || '—'}</td><td>${c.returnTime || '—'}</td><td><strong>${x.deliveries}</strong></td><td><strong>${x.delivered}</strong></td><td><strong>${x.returned}</strong></td><td>${number(x.km,2)} km</td><td>${fmtMinutes(x.minutes)}</td><td><div class="actions">${!c.returnTime?`<button class="btn primary small" data-action="close-cycle" data-id="${c.id}">Registrar retorno</button>`:''}<button class="btn secondary small" data-action="manage-cycle-deliveries" data-id="${c.id}">Gerenciar entregas</button><button class="btn secondary small" data-action="edit-cycle" data-id="${c.id}">Ajustar</button><button class="btn danger small" data-action="delete-record" data-type="cycle" data-id="${c.id}">Apagar</button></div></td>
    </tr>`; }).join('')}</tbody></table></div>`;
  }
  ...
  async function startScheduledDelivery(id) {
    ...
    const child = {
      ...
      date:source.scheduledDate, orderNo:source.orderNo, coupon:source.coupon, docNo:source.docNo || '', cashierNo:source.cashierNo || '', customerName:uppercaseName(source.customerName), customerPhone:formatPhoneBR(source.customerPhone), purchaseTime:'', neighborhoodId:source.neighborhoodId, fee:source.fee,
  ...
  function openConfigModal(id='') {
    ...
    $('#configForm').addEventListener('submit',async e=>{
      e.preventDefault();const data=Object.fromEntries(new FormData(e.target).entries());
      if(configTab==='neighborhoods') data.name=uppercaseName(data.name);
  ...
  function exportExcelReport() {
    ...
    const sheets={
      ...
      CICLOS:[['Data','Ciclo','Tipo','Veículo','Entregador','Saída','Retorno','Entregas levadas','Entregas concluídas','Entregas que voltaram','KM Médio por Ciclo','Tempo Min','Receita'],...cycles.map(c=>{const x=cycleCalc(c);return[c.date,c.code,c.autoGenerated?'AUTOMÁTICO':'MANUAL',vehicle(c.vehicleId)?.name||'',employee(c.driverId)?.name||'',c.departureTime,c.returnTime,x.deliveries,x.delivered,x.returned,x.km,x.minutes,x.revenue]})],
