import { ensureSeed, saveAutoBackup, enableWriteThroughAutoBackup } from './db.js?v=5.4';
import { $, $$, toast, initTooltips, animateStatCards, performanceProfile } from './helpers.js?v=5.4';
import * as V from './views.js?v=5.4';
...
async function openOperatorPicker() {
  const { Collaborators } = await import('./db.js?v=5.4');
...
async function updateBadges() {
  const { Deliveries, Cycles } = await import('./db.js?v=5.4');
...
if ('serviceWorker' in navigator) {
  ...
  window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=5.4', { updateViaCache: 'none' }).then((registration) => registration.update()).catch(() => {}));
}
