(() => {
  ...
  let pcSyncStatus = { connected:false, syncing:false, lastSync:'', error:'', serverUrl:'' };
  let syncTimer = null;
  ...
  function defaultState() {
    ...
    return {
      ...
      settings: {
        ...
        pcServerUrl: '', pcSyncEnabled: false
      },
      vehicles: [
  ...
  function sameOriginPcServerUrl() {
    if (!location || !/^https?:$/.test(location.protocol)) return '';
    if (location.hostname.endsWith('github.io')) return '';
    return location.origin;
  }

  function configuredPcServerUrl() {
    return normalizeServerUrl(state?.settings?.pcServerUrl || sameOriginPcServerUrl());
  }

  function pcSyncEnabled() {
    return Boolean(state?.settings?.pcSyncEnabled || sameOriginPcServerUrl());
  }
  ...
  async function pcApi(path, options = {}, timeoutMs = 3500) {
    const base = configuredPcServerUrl();
    if (!base) throw new Error('Servidor do PC não configurado');
  ...
  async function checkPcServer(silent = true) {
    if (!pcSyncEnabled()) {
      pcSyncStatus = { connected:false, syncing:false, lastSync:pcSyncStatus.lastSync || '', error:'', serverUrl:'' };
      updateConnectionStatus();
      return false;
    }
    const url = configuredPcServerUrl();
    try {
      const info = await pcApi('/api/health', {}, 2500);
      pcSyncStatus.connected = true;
      pcSyncStatus.error = '';
      pcSyncStatus.serverUrl = url;
      pcSyncStatus.serverInfo = info;
      updateConnectionStatus();
      return true;
    } catch (err) {
      pcSyncStatus.connected = false;
      pcSyncStatus.error = err?.message || String(err);
      pcSyncStatus.serverUrl = url;
      updateConnectionStatus();
      if (!silent) toast('Não foi possível conectar ao PC. Verifique se o servidor está aberto e se o aparelho está na mesma rede.','error');
      return false;
    }
  }

  async function pushStateToPc({silent=true} = {}) {
    if (!pcSyncEnabled() || pcSyncStatus.syncing) return false;
    pcSyncStatus.syncing = true;
    updateConnectionStatus();
    try {
      const result = await pcApi('/api/state', { method:'PUT', body:JSON.stringify(state) }, 6500);
      pcSyncStatus.connected = true;
      pcSyncStatus.lastSync = result?.savedAt || nowISO();
      pcSyncStatus.error = '';
      pcSyncStatus.serverUrl = configuredPcServerUrl();
      if (!silent) toast('Dados enviados para a memória do PC com sucesso.','success');
      return true;
    } catch (err) {
      pcSyncStatus.connected = false;
      pcSyncStatus.error = err?.message || String(err);
      if (!silent) toast('Não foi possível salvar no PC agora. Os dados continuam protegidos neste aparelho e poderão ser sincronizados depois.','warning');
      return false;
    } finally {
      pcSyncStatus.syncing = false;
      updateConnectionStatus();
    }
  }

  async function pullStateFromPc({force=false, silent=true} = {}) {
    if (!pcSyncEnabled()) return false;
    try {
      const payload = await pcApi('/api/state', {}, 5000);
      const remote = payload?.state;
      if (!remote) return false;
      const remoteTime = String(remote?.meta?.updatedAt || '');
      const localTime = String(state?.meta?.updatedAt || '');
      if (!force && remoteTime && localTime && remoteTime <= localTime) return false;
      state = migrateState(remote);
      await idbSet(STATE_KEY, state);
      pcSyncStatus.connected = true;
      pcSyncStatus.lastSync = payload?.servedAt || nowISO();
      pcSyncStatus.error = '';
      refreshYearOptions();
      refreshWeekOptions();
      render();
      updateBadges();
      if (!silent) toast('Dados carregados da memória do PC.','success');
      return true;
    } catch (err) {
      pcSyncStatus.connected = false;
      pcSyncStatus.error = err?.message || String(err);
      updateConnectionStatus();
      if (!silent) toast('Não foi possível carregar os dados do PC.','error');
      return false;
    }
  }

  async function reconcileWithPc() {
    if (!pcSyncEnabled() || pcSyncStatus.syncing) return;
    const ok = await checkPcServer(true);
    if (!ok) return;
    try {
      const payload = await pcApi('/api/state', {}, 5000);
      const remote = payload?.state;
      if (!remote) { await pushStateToPc({silent:true}); return; }
      const remoteTime = String(remote?.meta?.updatedAt || '');
      const localTime = String(state?.meta?.updatedAt || '');
      if (remoteTime > localTime) {
        ...
        await pushStateToPc({silent:true});
      }
    } catch (err) {
      pcSyncStatus.connected = false;
      pcSyncStatus.error = err?.message || String(err);
  ...
  function startPcSyncLoop() {
    if (syncTimer) clearInterval(syncTimer);
    syncTimer = setInterval(() => { if (!document.hidden) reconcileWithPc(); }, 15000);
    document.addEventListener('visibilitychange', () => { if (!document.hidden) reconcileWithPc(); });
  }
  ...
  async function saveState(action = '') {
    ...
    await idbSet(STATE_KEY, state);
    updateBadges();
    if (pcSyncEnabled()) await pushStateToPc({silent:true});
  }
  ...
  async function initialize() {
    try {
      ...
      // Quando aberto diretamente pelo servidor do PC, ele vira a fonte principal dos dados.
      // No APK/PWA, o usuário pode configurar o endereço do PC em Cadastros > Backup e memória.
      if (pcSyncEnabled()) {
        const ok = await checkPcServer(true);
        if (ok) {
          const payload = await pcApi('/api/state', {}, 5000).catch(() => null);
          if (payload?.state) {
            const remoteTime = String(payload.state?.meta?.updatedAt || '');
            const localTime = String(state?.meta?.updatedAt || '');
            if (!stored || remoteTime >= localTime) state = migrateState(payload.state);
            else await pushStateToPc({silent:true});
          } else if (stored) {
            await pushStateToPc({silent:true});
          }
        }
      ...
      if (!stored && !pcSyncStatus.connected) await saveState('Sistema inicializado');
      else if (autoCycleInit.changed) await saveState(`V14: ${autoCycleInit.cyclesCreated} ciclo(s) automático(s) identificado(s) e ${autoCycleInit.deliveriesLinked} entrega(s) vinculada(s)`);
      else await idbSet(STATE_KEY, state);

      bindStaticEvents();
      initPWA();
      updateConnectionStatus();
      window.addEventListener('online', () => { updateConnectionStatus(); reconcileWithPc(); });
  ...
  function updateConnectionStatus() {
    const dot = $('#connectionDot'), title = $('#connectionTitle'), subtitle = $('#connectionSubtitle');
    if (!dot || !title || !subtitle) return;
    if (pcSyncEnabled()) {
      dot.className = `connection-dot ${pcSyncStatus.connected ? 'online' : 'offline'}`;
      if (pcSyncStatus.syncing) {
        title.textContent = `Salvando no PC • ${modeLabel()}`;
        subtitle.textContent = 'Aguarde um instante...';
      } else if (pcSyncStatus.connected) {
        title.textContent = `PC conectado • ${modeLabel()}`;
        subtitle.textContent = 'Memória central no computador • sem internet';
      } else {
        title.textContent = `PC indisponível • ${modeLabel()}`;
        subtitle.textContent = 'Você pode continuar; o aparelho mantém uma cópia local';
      }
      return;
    }
    const online = navigator.onLine;
    dot.className = `connection-dot ${online ? 'online' : 'offline'}`;
    title.textContent = `Somente neste aparelho • ${modeLabel()}`;
    subtitle.textContent = 'Ative a memória do PC em Cadastros > Backup e memória';
  }
  ...
  function systemYears() {
    const current = new Date().getFullYear();
    const years = new Set();
    // Calendário ampliado e futuro: 2024 até pelo menos 2050.
    // Depois disso, cresce automaticamente para manter 25 anos futuros disponíveis.
    const maxYear = Math.max(2050, new Date().getFullYear() + 25);
    for (let year = 2024; year <= maxYear; year++) years.add(year);
    years.add(current);
  ...
  function settingsContent() {
    ...
    if (configTab === 'data') {
      const serverUrl = state.settings.pcServerUrl || '';
      const serverState = pcSyncEnabled() ? (pcSyncStatus.connected ? 'Conectado' : 'Desconectado') : 'Desativado';
      const statusClass = pcSyncStatus.connected ? 'green' : (pcSyncEnabled() ? 'red' : 'gray');
      return `${sectionHeader('▣','Backup e memória do PC','Use o computador como memória central sem depender de internet. O celular/APK e o PC trabalham pela mesma rede local.')}
        <div class="pc-storage-grid">
          <section class="pc-storage-card">
            <div class="pc-storage-head"><div><span class="eyebrow">MEMÓRIA CENTRAL</span><h3>Servidor local no seu PC</h3></div><span class="badge ${statusClass}">${serverState}</span></div>
            <p>Quando ativado, cada lançamento é salvo no aparelho e também no computador. Internet não é necessária; basta o PC estar ligado e os aparelhos estarem na mesma rede local.</p>
            <div class="form-grid pc-server-form">
              <label class="full">Endereço do PC<input id="pcServerUrlInput" type="text" value="${attr(serverUrl)}" placeholder="Ex.: http://192.168.1.50:8765" /></label>
              <label class="checkbox-line full"><input id="pcSyncEnabledInput" type="checkbox" ${state.settings.pcSyncEnabled ? 'checked' : ''} /><span><strong>Usar meu PC como memória central</strong><small>O aparelho continua com uma cópia local para emergências.</small></span></label>
            </div>
            <div class="form-actions pc-server-actions" style="justify-content:flex-start">
              <button class="btn primary" data-action="save-pc-server">Salvar e testar conexão</button>
              <button class="btn secondary" data-action="pull-from-pc" ${pcSyncEnabled()?'':'disabled'}>↓ Carregar dados do PC</button>
              <button class="btn secondary" data-action="push-to-pc" ${pcSyncEnabled()?'':'disabled'}>↑ Enviar dados deste aparelho</button>
            </div>
            <div class="pc-storage-status"><strong>Endereço atual:</strong> ${esc(configuredPcServerUrl() || 'Não configurado')}<br><strong>Última sincronização:</strong> ${pcSyncStatus.lastSync ? dateTimeBR(pcSyncStatus.lastSync) : 'Ainda não houve'}${pcSyncStatus.error ? `<br><strong>Último erro:</strong> ${esc(pcSyncStatus.error)}` : ''}</div>
          </section>
          <section class="pc-storage-card secondary-storage">
            <div class="pc-storage-head"><div><span class="eyebrow">CÓPIA DE SEGURANÇA</span><h3>Backup manual JSON</h3></div></div>
            <p>Use antes de grandes atualizações ou para guardar uma cópia extra. O backup inclui entregas, ciclos, KM, custos, cadastros, treinamento e histórico.</p>
  ...
  function bindSettingsActions() {
    ...
    $$('[data-action="save-pc-server"]').forEach(b=>b.addEventListener('click',savePcServerSettings));
    $$('[data-action="pull-from-pc"]').forEach(b=>b.addEventListener('click',()=>pullStateFromPc({force:true,silent:false})));
    $$('[data-action="push-to-pc"]').forEach(b=>b.addEventListener('click',()=>pushStateToPc({silent:false})));
  ...
  async function savePcServerSettings() {
    const url = normalizeServerUrl($('#pcServerUrlInput')?.value || '');
    const enabled = Boolean($('#pcSyncEnabledInput')?.checked);
    state.settings.pcServerUrl = url;
    state.settings.pcSyncEnabled = enabled;
    await idbSet(STATE_KEY, state);
    pcSyncStatus.serverUrl = url;
    if (!enabled) {
      pcSyncStatus.connected = false; pcSyncStatus.error = ''; updateConnectionStatus(); renderSettings();
      toast('Memória do PC desativada. Os dados continuam neste aparelho.','warning');
      return;
    }
    if (!url) { toast('Informe o endereço do PC, por exemplo: http://192.168.1.50:8765','error'); return; }
    const ok = await checkPcServer(false);
    if (ok) {
      const payload = await pcApi('/api/state', {}, 5000).catch(() => null);
      if (!payload?.state) await pushStateToPc({silent:true});
      toast('PC conectado. A memória central está ativa e não depende de internet.','success');
    }
    renderSettings();
  }
