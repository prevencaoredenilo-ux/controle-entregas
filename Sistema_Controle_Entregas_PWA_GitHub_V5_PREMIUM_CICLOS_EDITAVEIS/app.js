(() => {
  ...
  function cycleCalc(c) {
    ...
    const carriedDeliveries = state.deliveries.filter(d => d.cycleId === c.id);
  ...
  function vehicleDayStats(date, vehicleId) {
    ...
    const carried = state.deliveries.filter(x => x.date === date && x.vehicleId === vehicleId && x.cycleId);
  ...
  function activeCycleCards(cycles) {
    ...
    return `<div class="active-cycle-grid">${ordered.map(c=>{
      const x=cycleCalc(c); const linked=state.deliveries.filter(d=>d.cycleId===c.id);
      ...
      return `<article class="cycle-status-card ${c.returnTime?'closed':'open'}">
        ...
        <div class="cycle-status-actions">${!c.returnTime?`<button class="btn primary small" data-action="close-cycle" data-id="${c.id}">🏪 Registrar retorno</button>`:''}<button class="btn secondary small" data-action="manage-cycle-deliveries" data-id="${c.id}">▣ Gerenciar entregas</button><button class="btn secondary small" data-action="edit-cycle" data-id="${c.id}">Ajustar</button></div>
      </article>`;
    }).join('')}</div>`;
  }
  ...
  function cycleTable(list) {
    ...
    return `<div class="table-wrap"><table><thead><tr><th>Data</th><th>Ciclo</th><th>Veículo</th><th>Entregador</th><th>Saída</th><th>Retorno</th><th>Entregas levadas</th><th>Média KM/ciclo do dia</th><th>Tempo</th><th>Ações</th></tr></thead><tbody>${list.map(c => { const x=cycleCalc(c); return `<tr>
      <td>${dateBR(c.date)}</td><td><div class="cell-title">${esc(c.code)}</div>${c.returnTime?'<div class="cell-sub">Fechado</div>':'<div class="cell-sub" style="color:#A66A00">Em rota</div>'}</td><td>${esc(vehicle(c.vehicleId)?.name || '—')}</td><td>${esc(employee(c.driverId)?.name || '—')}</td><td>${c.departureTime || '—'}</td><td>${c.returnTime || '—'}</td><td><strong>${x.deliveries}</strong></td><td>${number(x.km,2)} km</td><td>${fmtMinutes(x.minutes)}</td><td><div class="actions">${!c.returnTime?`<button class="btn primary small" data-action="close-cycle" data-id="${c.id}">Registrar retorno</button>`:''}<button class="btn secondary small" data-action="manage-cycle-deliveries" data-id="${c.id}">Gerenciar entregas</button><button class="btn secondary small" data-action="edit-cycle" data-id="${c.id}">Ajustar</button></div></td>
    </tr>`; }).join('')}</tbody></table></div>`;
  }
  ...
  function nextCycleCode(date = todayISO()) {
    ...
    const seq = state.cycles.filter(c=>c.date===date).length + 1;
  ...
  function openCycleDepartureModal(preselectDeliveryId='') {
    ...
    form.addEventListener('submit',async e=>{
      ...
      const openVehicleCycle=state.cycles.find(c=>c.date===date && c.vehicleId===vehicleId && !c.returnTime);
      ...
      const openDriverCycle=state.cycles.find(c=>c.date===date && c.driverId===driverId && !c.returnTime);
      ...
      const c={id:uid('cyc'),code:fd.get('code')||nextCycleCode(date),date,vehicleId,driverId,departureTime,returnTime:'',notes:`Ciclo aberto com ${ids.length} entrega(s).`,createdAt:nowISO(),updatedAt:nowISO()};
      state.cycles.push(c);
      ids.forEach(id=>{
        const d=state.deliveries.find(x=>x.id===id); if(!d)return;
      ...
      closeModal();
      const odo=state.odometerLogs.find(o=>o.date===date && o.vehicleId===vehicleId && Number(o.kmStart||0)>0);
  ...
  function openManageCycleDeliveriesModal(cycleId) {
    ...
    const linked = state.deliveries.filter(d => d.cycleId === c.id);
    ...
    const available = state.deliveries
  ...
  function openCycleModal(id='') {
    ...
    openModal('Ajustar ciclo','Edição avançada. O fluxo normal deve ser feito por Montar saída e Registrar retorno.',`
      <form id="cycleForm">
        ...
        <div class="form-actions"><button type="button" class="btn secondary" id="cancelCycleBtn">Cancelar</button><button type="submit" class="btn primary">Salvar ajuste</button></div>
      </form>
    `,'AJUSTE AVANÇADO');
    $('#cancelCycleBtn').addEventListener('click',closeModal);
    $('#cycleForm').addEventListener('submit',async e=>{
      e.preventDefault();const data=Object.fromEntries(new FormData(e.target).entries());
      data.id=c.id;data.createdAt=c.createdAt||nowISO();data.updatedAt=nowISO();Object.assign(existing,data);
