...
(() => {
  ...
  const NOTICE_KEY='nilo_v28_notice';
  let db=null;
  ...
  async function persistLargeConfig(id,enabled,planned){
    ...
    for(let i=0;i<12;i++){
      ...
      s.meta=s.meta||{};s.meta.updatedAt=nowISO();s.meta.v28OperationalFix=true;await writeState(s);
  ...
  async function persistTripEditsFromForm(form,deliveryId){
    ...
    if(changed){s.meta=s.meta||{};s.meta.updatedAt=nowISO();s.meta.v28OperationalFix=true;await writeState(s);queueNotice('Viagens atualizadas','Horários e tempos das viagens foram salvos.');}
  }
  ...
  async function waitAndApplyReturn(payload){
    for(let i=0;i<18;i++){
      await new Promise(r=>setTimeout(r,260));
      try{if(await applyReturnContinuations(payload)){setTimeout(()=>location.reload(),420);return}}catch(err){console.warn('[NILO V28] retorno pendente',err)}
    }
  }
