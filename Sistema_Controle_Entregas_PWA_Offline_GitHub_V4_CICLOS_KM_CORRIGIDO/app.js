(() => {
  ...
  function activeCycleCards(cycles) {
    ...
    return `<div class="active-cycle-grid">${ordered.map(c=>{
      ...
      return `<article class="cycle-status-card ${c.returnTime?'closed':'open'}">
        ...
        <div class="cycle-status-actions">${!c.returnTime?`<button class="btn primary small" data-action="close-cycle" data-id="${c.id}">🏪 Registrar retorno</button>`:''}<button class="btn secondary small" data-action="edit-cycle" data-id="${c.id}">Ajustar</button></div>
      </article>`;
    }).join('')}</div>`;
  }
  ...
  function cycleTable(list) {
    ...
    return `<div class="table-wrap"><table><thead><tr><th>Data</th><th>Ciclo</th><th>Veículo</th><th>Entregador</th><th>Saída</th><th>Retorno</th><th>Entregas levadas</th><th>Média KM/ciclo do dia</th><th>Tempo</th><th>Ações</th></tr></thead><tbody>${list.map(c => { const x=cycleCalc(c); return `<tr>
      <td>${dateBR(c.date)}</td><td><div class="cell-title">${esc(c.code)}</div>${c.returnTime?'<div class="cell-sub">Fechado</div>':'<div class="cell-sub" style="color:#A66A00">Em rota</div>'}</td><td>${esc(vehicle(c.vehicleId)?.name || '—')}</td><td>${esc(employee(c.driverId)?.name || '—')}</td><td>${c.departureTime || '—'}</td><td>${c.returnTime || '—'}</td><td><strong>${x.deliveries}</strong></td><td>${number(x.km,2)} km</td><td>${fmtMinutes(x.minutes)}</td><td><div class="actions">${!c.returnTime?`<button class="btn primary small" data-action="close-cycle" data-id="${c.id}">Registrar retorno</button>`:''}<button class="btn secondary small" data-action="edit-cycle" data-id="${c.id}">Ajustar</button></div></td>
    </tr>`; }).join('')}</tbody></table></div>`;
  }
