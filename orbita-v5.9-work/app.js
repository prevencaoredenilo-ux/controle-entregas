import { ensureSeed, saveAutoBackup, enableWriteThroughAutoBackup } from './db.js?v=5.9';
import { $, $$, toast, initTooltips, animateStatCards, performanceProfile } from './helpers.js?v=5.9';
import * as V from './views.js?v=5.9';
...
async function openOperatorPicker() {
  const { Collaborators } = await import('./db.js?v=5.9');
...
function showNextTickerQuestion(forcedQuestion = null) {
  ...
  if (meta) meta.innerHTML = `${q.difficulty} · ${q.cyclePosition}/${q.bankSize} <b>›</b>`;
...
async function updateBadges() {
  const { Deliveries, Cycles } = await import('./db.js?v=5.9');
...
if ('serviceWorker' in navigator) {
  ...
  window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=5.9', { updateViaCache: 'none' }).then((registration) => registration.update()).catch(() => {}));
}
