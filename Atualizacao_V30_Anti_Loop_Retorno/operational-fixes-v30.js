...
(() => {
  ...
  const NOTICE_KEY='nilo_v30_notice';
  const REOPEN_CYCLE_KEY='nilo_v30_reopen_cycle';
  ...
  async function waitAndApplyReturn(payload){
    for(let i=0;i<18;i++){
      ...
      }catch(err){console.warn('[NILO V30] retorno pendente',err)}
    }
  }
  ...
  async function repairReturnedWithoutRetryOnDemand(){
    try{
      ...
      for(const source of s.deliveries.slice()){
        ...
        source.history=Array.isArray(source.history)?source.history:[];
        if(!source.history.some(h=>h?.type==='v30_retry_released'))source.history.push({id:uid('evt'),type:'v30_retry_released',cycleId:source.cycleId||'',at:nowISO()});
        source.updatedAt=nowISO();
        s.deliveries.push(buildContinuation(source,s,{partial:false}));created++;
      }
      if(!created)return 0;
      s.meta=s.meta||{};s.meta.updatedAt=nowISO();s.meta.v30ReturnAvailability=true;
      await writeState(s);
      queueNotice('Entrega devolvida liberada',`${created} entrega(s) devolvida(s) ficaram disponíveis para montar uma nova saída, sem apagar a tentativa anterior.`);
      return created;
    }catch(err){console.warn('[NILO V30] reparo sob demanda pendente',err);return 0}
  }
