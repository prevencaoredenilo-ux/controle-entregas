...
(() => {
  ...
  const NOTICE_KEY='nilo_v27_notice';
  let db=null;
  ...
  async function persistLargeConfig(id,enabled,planned){
    ...
    for(let i=0;i<12;i++){
      ...
      s.meta=s.meta||{};s.meta.updatedAt=nowISO();s.meta.v27OperationalFix=true;await writeState(s);
  ...
  function injectNewLargeDeliveryFields(){
    ...
    const box=document.createElement('section');box.className='v26-large-block v26-new-large v27-large-easy';
    box.innerHTML=`<div class="v27-type-kicker">TIPO DA ENTREGA</div><div class="v26-large-block-head"><div><strong>Essa compra é uma entrega normal ou grande?</strong><small>Escolha aqui no lançamento. Se for grande, informe quantas viagens você prevê.</small></div></div>
      <div class="v27-type-options"><label class="v27-type-option selected"><input type="radio" name="v27NewDeliverySize" value="normal" checked/><span><strong>Entrega normal</strong><small>Uma única viagem.</small></span></label><label class="v27-type-option"><input type="radio" name="v27NewDeliverySize" value="large" data-v26-new-large-toggle/><span><strong>Entrega grande</strong><small>Precisará de 2 ou mais viagens.</small></span></label></div>
      <div class="v26-large-controls hidden" data-v26-new-large-controls><label>Quantas viagens estão previstas?<input type="number" min="2" max="20" step="1" value="2" data-v26-new-planned /></label><div class="note">Você só informa a quantidade agora. Os horários e o tempo de cada viagem são liberados conforme as viagens forem acontecendo.</div></div>
      <div data-v27-new-trip-preview></div>`;
    form.prepend(box);
    const large=box.querySelector('[value="large"]'),normal=box.querySelector('[value="normal"]'),controls=box.querySelector('[data-v26-new-large-controls]'),planned=box.querySelector('[data-v26-new-planned]'),preview=box.querySelector('[data-v27-new-trip-preview]');
    const render=()=>{const on=large?.checked;controls?.classList.toggle('hidden',!on);box.querySelectorAll('.v27-type-option').forEach(x=>x.classList.toggle('selected',x.querySelector('input')?.checked));if(preview)preview.innerHTML=on?tripProgressHTML({id:'__new__',rootId:'__new__'}, {deliveries:[],cycles:[]}, planned?.value||2,{projected:true}):'';scheduleContrastAudit()};
    large?.addEventListener('change',render);normal?.addEventListener('change',render);planned?.addEventListener('input',render);render();
  }
  ...
  async function persistTripEditsFromForm(form,deliveryId){
    ...
    if(changed){s.meta=s.meta||{};s.meta.updatedAt=nowISO();s.meta.v27OperationalFix=true;await writeState(s);queueNotice('Viagens atualizadas','Horários e tempos das viagens foram salvos.');}
  }
  ...
  async function waitAndApplyReturn(payload){
    for(let i=0;i<18;i++){
      await new Promise(r=>setTimeout(r,260));
      try{if(await applyReturnContinuations(payload)){setTimeout(()=>location.reload(),420);return}}catch(err){console.warn('[NILO V27] retorno pendente',err)}
    }
  }
