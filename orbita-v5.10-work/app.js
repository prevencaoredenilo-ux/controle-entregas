import { ensureSeed, saveAutoBackup, enableWriteThroughAutoBackup } from './db.js?v=5.10';
import { $, $$, toast, initTooltips, animateStatCards, performanceProfile } from './helpers.js?v=5.10';
import * as V from './views.js?v=5.10';
...
async function openOperatorPicker() {
  const { Collaborators } = await import('./db.js?v=5.10');
...
const QUESTION_HISTORY_KEYS = {
  prev: 'orbita_question_history_prev_v510',
  geral: 'orbita_question_history_geral_v510',
};
const QUESTION_NEXT_GROUP_KEY = 'orbita_question_next_group_v510';
...
function showNextTickerQuestion(forcedQuestion = null) {
  ...
  const hint = $('#knowledgeTickerHint');
  const isGeneral = q.group === 'CURIOSIDADE GERAL';
  const difficultyTone = difficultyClass(q.difficulty);
  if (questionEl) {
  ...
  if (difficulty) {
    ...
    difficulty.className = `nq-difficulty ${difficultyTone}`;
  }
  if (counter) counter.textContent = `${q.cyclePosition}/${q.bankSize}`;
  if (hint) hint.textContent = `${isGeneral ? 'Curiosidade Geral' : 'Prevenção de Perdas'} • troca automática • sem repetição • clique para ver a resposta`;
  if (btn) {
...
async function updateBadges() {
  const { Deliveries, Cycles } = await import('./db.js?v=5.10');
...
if ('serviceWorker' in navigator) {
  ...
  window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=5.10', { updateViaCache: 'none' }).then((registration) => registration.update()).catch(() => {}));
}
