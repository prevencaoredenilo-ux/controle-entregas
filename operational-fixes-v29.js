...
(() => {
  ...
  const NOTICE_KEY='nilo_v29_notice';
  let db=null;
  ...
  async function persistLargeConfig(id,enabled,planned){
    ...
    for(let i=0;i<12;i++){
      ...
      s.meta=s.meta||{};s.meta.updatedAt=nowISO();s.meta.v29OperationalFix=true;await writeState(s);
  ...
  async function persistTripEditsFromForm(form,deliveryId){
    ...
    if(changed){s.meta=s.meta||{};s.meta.updatedAt=nowISO();s.meta.v29OperationalFix=true;await writeState(s);queueNotice('Viagens atualizadas','Horários e tempos das viagens foram salvos.');}
  }
  ...
  async function waitAndApplyReturn(payload){
    for(let i=0;i<18;i++){
      await new Promise(r=>setTimeout(r,260));
      try{if(await applyReturnContinuations(payload)){setTimeout(()=>location.reload(),3200);return}}catch(err){console.warn('[NILO V29] retorno pendente',err)}
    }
  }


  async function repairTodaysReturnedWithoutRetry(){
    try{
      const s=await readState();if(!s?.deliveries)return false;
      const today=todayISO();let created=0;
      for(const source of s.deliveries.slice()){
        if(source.date!==today)continue;
        if(source.status!=='Devolvida'||source.returnedUndelivered!==true)continue;
        if(source.scheduledDate||['Reagendada','Programada','Cancelada','Retirada na loja'].includes(source.status))continue;
        if(s.deliveries.some(d=>d.parentId===source.id))continue;
        const chain=chainFor(source,s);
        if(chain.some(x=>x.finalizationTime||x.status==='Finalizada'))continue;
        const latest=chain.slice().sort((a,b)=>Number(a.attemptNo||1)-Number(b.attemptNo||1)||String(a.createdAt||'').localeCompare(String(b.createdAt||''))).at(-1);
        if(!latest||latest.id!==source.id)continue;
        source.nextAction='Disponível para nova tentativa de entrega';
        source.history=Array.isArray(source.history)?source.history:[];
        if(!source.history.some(h=>h?.type==='v29_retry_released'))source.history.push({id:uid('evt'),type:'v29_retry_released',cycleId:source.cycleId||'',at:nowISO()});
        source.updatedAt=nowISO();
        s.deliveries.push(buildContinuation(source,s,{partial:false}));created++;
      }
      if(!created)return false;
      s.meta=s.meta||{};s.meta.updatedAt=nowISO();s.meta.v29ReturnAvailability=true;
      await writeState(s);
      queueNotice('Entrega devolvida liberada',`${created} entrega(s) devolvida(s) de hoje ficaram disponíveis para montar uma nova saída, sem apagar a tentativa anterior.`);
      return true;
    }catch(err){console.warn('[NILO V29] reparo de devolvidas pendente',err);return false}
  }
  ...
  function init(){
    ...
    setTimeout(async()=>{if(await repairTodaysReturnedWithoutRetry())setTimeout(()=>location.reload(),3200)},1700);
