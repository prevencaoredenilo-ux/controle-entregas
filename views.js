import { Deliveries, Vehicles, Drivers, Collaborators, Neighborhoods, CostCategories, ReturnReasons, Cycles, OdometerLogs, Costs, DayClosures, AuditLog, Counters } from './db.js?v=4.2';
import { $, $$, money, dateBR, dateTimeBR, timeBR, escapeHtml, toast, badge, STATUS_META, guardClick, downloadCSV, downloadJSON, wirePhoneMask, animateStatCards, motivationalPhrase, performanceProfile, barChartSVG, lineChartSVG, thermometerHTML } from './helpers.js?v=4.2';
import { getEnv, getOperatorName, getOperatorRole, canPerform, closeModal, openModal, refreshApp } from './app.js?v=4.2';
import { exportFullExcelReport } from './excel-report.js?v=4.2';
...
export async function renderSettings() {
  ...
  const { listAutoBackups } = await import('./db.js?v=4.2');
...
export function wireSettingsEvents() {
  $$('.auto-restore-btn').forEach((btn) => btn.addEventListener('click', async () => {
    ...
    const { restoreAutoBackup } = await import('./db.js?v=4.2');
  ...
  $('#settingsBackupBtn')?.addEventListener('click', async () => {
    const data = await (await import('./db.js?v=4.2')).exportAll();
  ...
  $('#settingsRestoreInput')?.addEventListener('change', async (e) => {
    ...
    const currentBackup = await (await import('./db.js?v=4.2')).exportAll();
    downloadJSON(`orbita-backup-seguranca-antes-restauracao-${Date.now()}.json`, currentBackup);
    try {
      const text = await file.text();
      const data = JSON.parse(text);
      await (await import('./db.js?v=4.2')).importAll(data);
